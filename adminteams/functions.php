...
include_once "config.php";
 ...
 function crudMain(){
    ...
    <h1>Backend teams</h1>
    <nav>
		<a href='insert.php'>Toevoegen team</a>
    </nav><br>";
    echo $txt;
 ...
 // selecteer de rij van de opgeven id uit de table orderen
...
function updateRecord($row){
    ...
    $stmt->execute([
        ...
        ':finishTime'=>$row['finishTime'] !== '' ? $row['finishTime'] : null,
        ':completionTimeSeconds'=>$row['completionTimeSeconds'] !== '' ? $row['completionTimeSeconds'] : null,
        ':id'=>$row['id']
    ]);
...
function insertRecord($post){
    ...
    // Maak een query 
    $sql = "
        INSERT INTO " . CRUD_TABLE . " (teamName, startTime, finishTime, completionTimeSeconds)
        VALUES (:teamName, :startTime, :finishTime, :completionTimeSeconds) 
    ";
    ...
    $stmt->execute([
        ...
        ':finishTime'=>$_POST['finishTime'] !== '' ? $_POST['finishTime'] : null,
        ':completionTimeSeconds'=>$_POST['completionTimeSeconds'] !== '' ? $_POST['completionTimeSeconds'] : null
    ]);
