...
if ($team !== null) {
    ...
    $stmt = $db_connection->prepare("UPDATE teams SET startTime = NOW() WHERE id = :id AND startTime IS NULL");
    $stmt->execute([':id' => $team['teamId']]);

    startRoomTimerIfNeeded($db_connection, $team['teamId'], 'room1StartTime');

    // Team refresh voor room1starttime omdat het update hierboven
    $team = getUserTeam($db_connection, $_SESSION['user_id']);

    if (isRoomTimeUp($team['room1StartTime'], ROOM_DURATION_SECONDS)) {
...
$deadlineIso = ($team !== null) ? getRoomDeadlineIso($team['room1StartTime'], ROOM_DURATION_SECONDS) : null;
?>
